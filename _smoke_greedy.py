"""Smoke test for the radar_scripted='greedy' trainer path (no validation)."""
import sys, json
sys.path.insert(0, '.')
...
cfg = S2PPOConfigV2(
    ...
    seed=998, train_seed=998, device="cuda",
)
...
trainer = S7SelfPlayTrainer(
    ...
    out_dir=Path('experiments/array_face_s7/learning_repair/_smoke_greedy'),
...
for _ in range(2):
    ...
    print({k: round(v, 4) if isinstance(v, float) else v
           ...
           ('iteration', 'rollout_drop', 'rollout_success', 'jammer_entropy',
            'radar_entropy', 'jammer_clip_frac', 'radar_clip_frac')})
...
print('radar_entropy keys zero (update skipped):',
      m.get('radar_entropy'), m.get('radar_clip_frac'))
