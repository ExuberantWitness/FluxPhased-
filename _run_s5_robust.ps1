...
$chainlog = "$base\s5_chain.log"

$seed = 20260729
$out = "$base\s5_ippo_output_seed${seed}"
...
for ($r = 1; $r -le 30; $r++) {
  ...
  & $pyexe -u experiments\array_face_s5\learning_repair\run_s5_ippo.py --seed $seed --resume >> "$out\run.log" 2>&1
