"""Smoke tests for BC pretrainer (WP-3 M2: BlindClassical teacher).
...
WP-3 M2: Teacher swapped StrongRule → BlindClassical (blind API; no god-view leak).
"""
...
def test_bc_pretrainer_smoke():
    ...
    WP-3 M2: uses BlindClassicalCommander as teacher (blind API).
    """
    ...
    # WP-3 M2: buffer should have beam_direction (NOT legacy beam_target)
    assert "beam_direction" in samples, "BC buffer missing beam_direction"
    assert "beam_target" not in samples, "BC buffer should NOT have beam_target (god-view)"
...
def test_bc_changes_deterministic_policy():
    ...
    WP-3 M0/M2: get_action_for_env signature requires detect_list.
    """
    ...
    with torch.no_grad():
        action_before, _ = ac.get_action_for_env(
            obs_dict["obs"][:, 0], detect_0, obs_dict["privileged"][:, 0], deterministic=True)
