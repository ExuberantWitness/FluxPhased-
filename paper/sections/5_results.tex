...
The attacker-count axis extends beyond the matched control by training the n-jammer game at $n=3$ (sites $+60^\circ, 0^\circ, -60^\circ$, budget split 21/21/21) and $n=4$ (sites $+60^\circ,+20^\circ,-20^\circ,-60^\circ$, split 16/16/16/15) under the identical two-stage protocol, with the $n=2$ cross-fire run as the reference. Pre-training gate profiles computed with the exact published semantics reproduce the $n=2$ profile digit-for-digit and pass for both arc placements, so the extension stayed inside the validated regime. Across three converged seeds each, neutralization is $22.2\%\pm1.5\%$ at $n=3$ (h2h $0.224$, jam-vs-sweep $0.362$) and $21.1\%\pm4.1\%$ at $n=4$ (h2h $0.192$, jam-vs-sweep $0.328$)---both statistically indistinguishable from the $n=2$ value of $24.2\%$, while the raw disruption against scripted sweeps falls monotonically from $0.529$ through $0.362$ to $0.328$.

The scaling picture is therefore not monotone in attacker count at fixed total budget: the containment break occurs at the $1\to2$ step ($62.8\%$ to mid-20s), and neither a third nor a fourth jammer deepens it. The per-jammer budget, not the count, becomes the binding variable---21 and even 16 tokens per jammer dilute each site's peak disruptive power below what the pairwise equilibrium extracts from 31--32 tokens. For defense design this is the useful direction of the result: the stress test that matters is the transition from one source to two; beyond that, the antagonist's total energy budget dominates its distribution across platforms.
