...
- `status`: `PROVISIONAL`
- `reviewer_route`: `same-family independent reviewer`
- `evidence_cutoff`: `2026-07-28`
- `independence_boundary`: reviewer 与生成候选的流程相互独立，但属于同一模型家族；该判定不是外部同行评审、异构模型复核或实验验证。
- `claim_boundary`: 仅给出候选排序与 falsification 要求，不构成物理有效、headroom 存在、PPO 可学习或 G3-BSTA 成功的证据。

## Verdict

**暂定唯一推荐核心候选：A，Budgeted Sparse Beam–Target Allocation。**
...
所有分数均为 1–5，越高越好；最后一列是抗制造 headroom 能力，5 代表风险最低。物理列全部是 conditional judgment，不代表当前 M7 已通过物理验证。

| 候选 | 物理可辩护（conditional） | 非平凡因果决策 | 脚本公平性 | 训练前可证伪 | 实施范围可控 | 文献相对新颖性 | 抗制造 headroom |
|---|---:|---:|---:|---:|---:|---:|---:|
| **A** | **3 (conditional)** | **5** | **5** | **5** | 3 | 2 | **4** |
| B | 4 (conditional) | 2 | 5 | 5 | **5** | 1 | 2 |
| C | 3 (conditional) | 5 | 5 | 4 | 2 | 2 | 2 |
| D | 2 (conditional) | 4 | 4 | 4 | 1 | 3 | 1 |
| E | 3 (conditional) | 1 | 5 | 5 | 5 | 3 | 5 |

物理分仅在相应 emitter、receiver、传播/频谱、target selectivity、cross-talk 和 detector semantics 得到 symbol-level 解析及校准后成立。A 当前只给 3 分，因为“选择 target 会产生可区分的 receiver-local effect”尚未验证；若不存在该机制，A 直接被否决。E 不引入物理机制，其物理分是中性，而不是“当前物理已验证”。
...
- reduced exact DP 中必须存在至少一对相同 `(time, energy)`、但因果 observation history 不同的可达状态，其最优可行 target 集合不同；并且每个状态中被选最优 target 相对次优 target 的 action-value gap 必须超过预注册的 `epsilon_Q`、solver tolerance 和 Monte Carlo uncertainty，数值并列或不可分辨的 action 不计；
...
- 正式 power analysis 决定的独立 training seeds（且工程下限不低于 8）下，对任一 frozen script finalist 的一侧 95% LCB 不大于 `0.05`。
