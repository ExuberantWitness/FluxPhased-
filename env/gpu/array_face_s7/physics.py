...
def _service_tables(physics: DebugPhysicsConfig, *, device) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Cache service link-budget tensors, keyed by values/device."""
    services = (physics.service_0, physics.service_1)
    key = (str(device), tuple((float(s.fc_hz), float(s.bw_hz), float(s.rx_gain_db)) for s in services))
    tables = _SERVICE_CACHE.get(key)
    if tables is None:
        tables = tuple(torch.tensor([getattr(s, field) for s in services],
                                    device=device, dtype=torch.float32)
                       for field in ("fc_hz", "bw_hz", "rx_gain_db"))
        _SERVICE_CACHE[key] = tables
    return tables


def target_gain_table(radar_cfg: UPAConfig, *, device) -> torch.Tensor:
    """Cached [N_AZ, N_EL, N_AZ] target gain generated by the reference path."""
    key = (str(device), tuple(radar_cfg.beam_az_deg), tuple(radar_cfg.beam_el_deg),
           radar_cfg.n_cells_az, radar_cfg.n_cells_el)
    cache = getattr(target_gain_table, "_cache", {})
    if key not in cache:
        ba = torch.arange(5, device=device).view(5, 1, 1).expand(5, 5, 5).reshape(-1)
        be = torch.arange(5, device=device).view(1, 5, 1).expand(5, 5, 5).reshape(-1)
        ma = torch.arange(5, device=device).view(1, 1, 5).expand(5, 5, 5).reshape(-1)
        cache[key] = target_gain_db(radar_cfg, beam_az_idx=ba, beam_el_idx=be,
                                    mission_az_idx=ma).reshape(5, 5, 5)
        target_gain_table._cache = cache
    return cache[key]
...
def compute_jnr_db_s7(
    ...
    *,
    ...
    fc_table, bw_table, rx_gain_table = _service_tables(physics, device=device)

    n_active = cell_mask.sum(dim=-1).clamp(min=1).to(torch.float32)
