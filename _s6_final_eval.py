"""Final three-view evaluation of the S6 self-play checkpoint with fresh
action seeds (the training-log final row used action_seed=4242 only)."""
...
SEED = 20260729
device = 'cuda'
out_dir = Path('experiments/array_face_s6/learning_repair/s6_selfplay_output_seed20260729')
...
# NOTE: matches the regime the checkpoint was TRAINED in (stale overrides).
env_cfg = EnvConfig(
    n_envs=16, horizon=64, n_services=2,
    dt=1.0, P_jam_W=2.0, active_budget_steps=63, duty_budget=1.0,
...
def sweep_vs_idle(seed_subset, action_seed):
...
t0 = time.time()
floor_drop = sweep_vs_idle(val_seeds, 4242)
